<?php
	...
	$pagina_actual = "reporte-pago-generales";
	$fecha = date("Y-m-d");
				...
				<div class="main-content">
					...
					<div class="widget">
						...
						<div class="widget-content">
							...
							?>
							<form class="form-horizontal" role="form" method="post" action="reporte-pagos.php?b=1" autocomplete="off" enctype="multipart/form-data">
							<div class="row">
							    <div class="col-md-6">
									...
									<div class="form-group">
										...
										<div class="col-sm-9">
											<select class="form-control" required name="tipo-pago" id="tipo-pago">
												...
												<option value="interes-moratorio">Pago de Intereses Moratorios</option>
											</select>
										</div>
									</div>						
								</div>						
					...
					<?php
						if(!isset($_GET['b'])){
							...
							$consulta_perrona = "SELECT * FROM pagos_vista_simple WHERE(fecha_captura >= '".$fechaInicial." 00:00:00' AND fecha_captura <='".$fechaFinal." 23:59:59') ".implode(" ",$arreglo_condiciones)." ORDER BY folio_fisico desc;";	
							//CONSULTA PERRONA
							/*
							//echo "TODOS los metodos";
							if($metodo == "200"){
								
								if($tipo_pago == "capital"){
									 echo $consulta = "SELECT * FROM pagos_vista_simple WHERE(fecha_captura BETWEEN '".$fechaInicial."' AND '".$fechaFinal."') AND tipo_pago='2' ORDER BY folio_fisico desc;";
								}
								if($tipo_pago  == "interes"){
									 echo $consulta = "SELECT * FROM pagos_vista_simple WHERE(fecha_captura BETWEEN '".$fechaInicial."' AND '".$fechaFinal."') AND tipo_pago='1' ORDER BY folio_fisico desc;";
								}
								if($tipo_pago == "todos"){
									 echo $consulta = "SELECT * FROM pagos_vista_simple WHERE(fecha_captura BETWEEN '".$fechaInicial."' AND '".$fechaFinal."') ORDER BY folio_fisico desc;";
								}
							}elseif($metodo == "300"){
								
								//echo "todos lo metodos excepto efectivo";
								if($tipo_pago == "capital"){
									echo $consulta = "SELECT * FROM pagos_vista_simple WHERE(fecha_captura BETWEEN '".$fechaInicial."' AND '".$fechaFinal."') AND tipo_pago='2' AND metodo_pago NOT IN ('100') ORDER BY folio_fisico desc;";	
								}
								if($tipo_pago == "interes"){
									echo $consulta = "SELECT * FROM pagos_vista_simple WHERE(fecha_captura BETWEEN '".$fechaInicial."' AND '".$fechaFinal."') AND tipo_pago='1' AND metodo_pago NOT IN ('100') ORDER BY folio_fisico desc;";	
								}
								if($tipo_pago == "todos"){
									echo $consulta = "SELECT * FROM pagos_vista_simple WHERE(fecha_captura BETWEEN '".$fechaInicial."' AND '".$fechaFinal."') AND metodo_pago NOT IN ('100') ORDER BY folio_fisico desc;";
								}
								
							}else{
								
								if($tipo_pago == "capital"){
									echo $consulta = "SELECT * FROM pagos_vista_simple WHERE(fecha_captura BETWEEN '".$fechaInicial."' AND '".$fechaFinal."') AND tipo_pago='2' AND metodo_pago = '".$metodo."' ORDER BY folio_fisico desc;";	
								}
								if($tipo_pago == "interes"){
									echo $consulta = "SELECT * FROM pagos_vista_simple WHERE(fecha_captura BETWEEN '".$fechaInicial."' AND '".$fechaFinal."') AND tipo_pago='1' AND metodo_pago = '".$metodo."' ORDER BY folio_fisico desc;";	
								}
								if($tipo_pago == "todos"){
									echo $consulta = "SELECT * FROM pagos_vista_simple WHERE(fecha_captura BETWEEN '".$fechaInicial."' AND '".$fechaFinal."') AND metodo_pago = '".$metodo."' ORDER BY folio_fisico desc;";
								}
								
								//echo "solo 1 metodo";
							}*/
							$iny_consulta = mysqli_query($mysqli,$consulta_perrona) or die (mysqli_error());
							if(mysqli_num_rows($iny_consulta) > 0){
								...
								<div class="widget-content">
										...
										<table id="tablita" class="table table-bordered datatable">
											<thead>
												<tr>
													<th>ID</th>
													<th>Folio Fisico</th>
								...
								while($row = mysqli_fetch_assoc($iny_consulta)){
									...
									<tr>
									<td>'.$row['id_pagos'].'</td>
									<td><span class="badge">'.$row['folio_fisico'].'</span></td>
									<td>#'.$row['folio'].'</td>
									...
									if($row['tipo_pago'] == 4)	
										echo "<td><span class='label label-warning'>Pago de Int. Moratorio</span></td>";

									echo '<td>'.$row['comentarios'].'</td>';
									
									if($row['metodo_pago'] == 100){
										echo "<td><span class='badge'><i class='fa fa-money' aria-hidden='true'></i> Efectivo</span></td>";
									}elseif($row['metodo_pago'] == 0){
										echo "<td><i class='fa fa-question' aria-hidden='true'></i> No Info</td>";
									}else{
										echo "<td><span class='badge'><i class='fa fa-university' aria-hidden='true'></i> ".$row['banco']."</span><br><i class='fa fa-male' aria-hidden='true'></i> ".$row['titular']."</td>";
									}
									echo "<td>".$row['nombre_inversionista']."</td>";
								echo '</tr>
