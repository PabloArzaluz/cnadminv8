...
<head>
		...
		function alertaCreditoJuridico(id){
			...
			<?php
				...
				while ($fila_juridico_conocer = mysqli_fetch_array($conocer_en_juridico)) {
					...
					echo "];";
				//Vendido
				$conocer_vendido = mysqli_query($mysqli,"select id_creditos from creditos where status=4 ;") or die(mysqli_error());
				echo "var myArrVend= [";
				while ($fila_vendido_conocer = mysqli_fetch_array($conocer_vendido)) {
	...
	<script type="text/javascript">
		//Obtener informacion del Credito al seleccionarlo desde el Select
...
<script type="text/javascript">
        $(document).ready(function() {  
            $('#monto-pago').blur(function(){
				...
				if(document.getElementById('tipo-pago').value == 1){
					$.ajax({
                    type: "POST",
                    url: "ajax/return_data_pago_intereses.php",
                    data: dataString,
                    success: function(response) {
                     if (!$.trim(response)){   
							...
							var data = response;
							var data_receive = JSON.parse(data);
							var monto_pago_regular = (Math.round(data_receive.monto_pagar_regular * 100) / 100).toFixed(2);
							var excedente = (Math.round(data_receive.excedente * 100) / 100).toFixed(2);
							...
							$("#InfoMonto").html(`<div class="alert alert-info" role="alert">
													Pago regular de Intereses: <b>$ `+monto_pago_regular+`</b>.<br>
													`+mensaje_excedente+`
													</div>`);
						}  
                    }
...
<script>
	$(document).ready(function() { 
		//Bloquear Control de Tipo de pago hasta que el Credito sea seleccionado
		document.getElementById("tipo-pago").disabled = true;
		document.getElementById("tipo-pago").disabled = true;
		document.getElementById('monto-pago').setAttribute('disabled', 'disabled');
		var select_credito = document.getElementById("credito");
		//Al seleccionar el credito se habilitar el campo de tipo de pago
		select_credito.onchange = function () {
			if (this.value != "" || this.value.length > 0) {
				document.getElementById('monto-pago').value= "";
				document.getElementById('monto-pago').setAttribute('disabled', 'disabled');
				document.getElementById("tipo-pago").disabled = false;
			}else{
				document.getElementById("tipo-pago").disabled = true;
				document.getElementById("tipo-pago").selectedIndex = 0;
		...
		var select_tipo_pago = document.getElementById("tipo-pago");
		//Al seleccionar el credito se habilitar el campo de tipo de pago
...
<script>
	//Avoid Scrollin on input number amount
					...
					<div class="widget">
						...
						<div class="widget-content">
							...
							<div class="row">
                                ...
                                <form class="form-horizontal" role="form" method="post" action="_registro_nuevo_pago_v2.php" enctype="multipart/form-data">
												...
												<!-- SELECT2 -->
														<select name="credito" id="credito" class="select2" required onchange="alertaCreditoJuridico(this.value)">
                                                            ...
                                                            ON
                                                                ...
                                                                where creditos.status = 1 OR creditos.status =3 OR creditos.status=4 order by 1 desc;") or die (mysqli_error());
										...
										<div class="form-group">
											...
											<div class="col-md-9">
												<div class="input-group">
													...
													<input type="number"  step="0.01" min="0" class="form-control noscroll" disabled="disabled" id="monto-pago" name="monto-pago" placeholder="Monto Pago" required>
												</div>
											</div>
